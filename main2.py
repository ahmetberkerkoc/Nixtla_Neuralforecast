...
from neuralforecast.models import DeepAR, Informer, iTransformer, NBEATS, NHITS, TimeMixer
...
plt.plot(forecast_df['ds'], forecast_df[model_name]*(10**7), label="Forecast", color='red', linestyle='dashed', linewidth=2)
plt.axvline(test_df['ds'].iloc[0], color='gray', linestyle='--', label='Test Start')
plt.xlabel("Date")
plt.ylabel("Gas Consumption (ABONE)")
plt.title(f"{model_name} Forecast on Test Set")
plt.legend()
